...
describe UsersController do
  describe "route generation" do
    ...
    it "should map { :controller => 'users', :action => 'update', :id => 1} to /usuarios/1" do
      route_for(:controller => "users", :action => "update", :id => 1).should == "/usuarios/1"
    end
  ...
  describe "route recognition" do
    ...
    it "should generate params { :controller => 'users', action => 'index' } from GET /usuarios" do
      params_from(:get, "/usuarios").should == {:controller => "users", :action => "index"}
    end

    it "should generate params { :controller => 'users', action => 'new' } from GET /usuarios/novo" do
      params_from(:get, "/usuarios/novo").should == {:controller => "users", :action => "new"}
    end

    it "should generate params { :controller => 'users', action => 'create' } from POST /usuarios" do
      params_from(:post, "/usuarios").should == {:controller => "users", :action => "create"}
    end

    it "should generate params { :controller => 'users', action => 'show', id => '1' } from GET /usuarios/1" do
      params_from(:get, "/usuarios/1").should == {:controller => "users", :action => "show", :id => "1"}
    end

    it "should generate params { :controller => 'users', action => 'edit', id => '1' } from GET /usuarios/1;edit" do
      params_from(:get, "/usuarios/1/ver").should == {:controller => "users", :action => "edit", :id => "1"}
    end

    it "should generate params { :controller => 'users', action => 'update', id => '1' } from PUT /usuarios/1" do
      params_from(:put, "/usuarios/1").should == {:controller => "users", :action => "update", :id => "1"}
    end

    it "should generate params { :controller => 'users', action => 'destroy', id => '1' } from DELETE /usuarios/1" do
